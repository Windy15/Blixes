...
do
	...
	local function AcquireRunnerThreadAndCallSignalHandler(fn, ...)
	...
	local function RunSignalHandlerInFreeThread(...)
		AcquireRunnerThreadAndCallSignalHandler(...)
		while true do
			AcquireRunnerThreadAndCallSignalHandler(coroutine.yield())
		end
	end
	...
	function ScriptConnection:Disconnect()
		...
		if self._disconnect_listener ~= nil then
			if not FreeRunnerThread then
				FreeRunnerThread = coroutine.create(RunSignalHandlerInFreeThread)
			end
...
local function WaitForLiveAccessCheck() -- This function was created to prSignal the ProfileService module yielding execution when required
	...
	for _, new_global_update in ipairs(new_global_updates_data[2]) do
		...
		if is_new == true then
			...
			if new_global_update[3] == false then
				-- Check if update is not pending to be locked: (PrSignaling firing new active update listeners more than necessary)
			...
			if new_global_update[3] == true then
				-- Check if update is not pending to be cleared: (PrSignaling firing new locked update listeners after marking a locked update for clearing)
...
local function SaveProfileAsync(profile, release_from_session, is_overwriting)
	...
	-- Compare "SessionLoadCount" when writing to profile to prSignal a rare case of repeat last save when the profile is loaded on the same server again
	...
	while repeat_save_flag == true do
		...
		elseif repeat_save_flag == true then
			task.wait() -- PrSignal infinite loop in case DataStore API does not yield
		end
	end
	ActiveProfileSaveJobs = ActiveProfileSaveJobs - 1
end
...
function Profile:Save()
	...
	-- Reject save request if a save is already pending in the queue - this will prSignal the user from
...
function ProfileStore:LoadProfileAsync(profile_key, not_released_handler, _use_mock) --> [Profile / nil] not_released_handler(place_id, game_job_id)
	...
	while ProfileService.ServiceLocked == false do
		...
		-- yoink the DataStore return for the new call. The older call will return nil. This would prSignal very rare
				...
				if IsThisSession(active_session) == true then
					...
					if force_load == true then
						...
						if force_load_uninterrupted == true then
							...
							task.wait() -- Overload prSignalion
						else
